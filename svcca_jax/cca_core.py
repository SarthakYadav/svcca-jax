...
Ijnputs have shape (num_neurons1, m), (num_neurons2, m). This can be directly
...
def positivedef_matrix_sqrt(array):
  ...
  #  A - jnp.dot(v, jnp.dot(jnp.diag(w), v.T))
  wsqrt = jnp.sqrt(w)
  sqrtarray = jnp.dot(v, jnp.dot(jnp.diag(wsqrt), jnp.conj(v).T))
  return sqrtarray
...
def remove_small(sigma_xx, sigma_xy, sigma_yx, sigma_yy, epsilon):
  ...
  x_diag = jnp.abs(jnp.diagonal(sigma_xx))
  y_diag = jnp.abs(jnp.diagonal(sigma_yy))
...
def compute_ccas(sigma_xx, sigma_xy, sigma_yx, sigma_yy, epsilon,
  ...
  Returns:
            ...
            x_idxs:       The indexes of the ijnput sigma_xx that were pruned
                          by remove_small
  ...
  if numx == 0 or numy == 0:
    return ([0, 0, 0], [0, 0, 0], jnp.zeros_like(sigma_xx),
            jnp.zeros_like(sigma_yy), x_idxs, y_idxs)

  if verbose:
    print("adding eps to diagonal and taking inverse")
  sigma_xx += epsilon * jnp.eye(numx)
  sigma_yy += epsilon * jnp.eye(numy)
  inv_xx = jnp.linalg.pinv(sigma_xx)
  inv_yy = jnp.linalg.pinv(sigma_yy)
  ...
  if verbose:
    print("dot products...")
  arr = jnp.dot(invsqrt_xx, jnp.dot(sigma_xy, invsqrt_yy))

  if verbose:
    print("trying to take final svd")
  u, s, v = jnp.linalg.svd(arr)

  if verbose:
    print("computed everything!")

  return [u, jnp.abs(s), v], invsqrt_xx, invsqrt_yy, x_idxs, y_idxs
...
def sum_threshold(array, threshold):
  ...
  Returns:
            i: index at which jnp.sum(array[:i]) >= threshold
  """
  assert (threshold >= 0) and (threshold <= 1), "print incorrect threshold"

  for i in range(len(array)):
    if jnp.sum(array[:i])/jnp.sum(array) >= threshold:
      return i
...
def create_zero_dict(compute_dirns, dimension):
  ...
  """
  return_dict = {}
  return_dict["mean"] = (jnp.asarray(0), jnp.asarray(0))
  return_dict["sum"] = (jnp.asarray(0), jnp.asarray(0))
  return_dict["cca_coef1"] = jnp.asarray(0)
  return_dict["cca_coef2"] = jnp.asarray(0)
  return_dict["idx1"] = 0
  return_dict["idx2"] = 0

  if compute_dirns:
    return_dict["cca_dirns1"] = jnp.zeros((1, dimension))
    return_dict["cca_dirns2"] = jnp.zeros((1, dimension))
...
def get_cca_similarity(acts1, acts2, epsilon=0., threshold=0.98,
  ...
  assert acts1.shape[0] < acts1.shape[1], ("ijnput must be number of neurons"
                                           "by datapoints")
  return_dict = {}
  ...
  covariance = jnp.cov(acts1, acts2)
  ...
  xmax = jnp.max(jnp.abs(sigmaxx))
  ymax = jnp.max(jnp.abs(sigmayy))
  sigmaxx /= xmax
  sigmayy /= ymax
  sigmaxy /= jnp.sqrt(xmax * ymax)
  sigmayx /= jnp.sqrt(xmax * ymax)
  ...
  if (not jnp.any(x_idxs)) or (not jnp.any(y_idxs)):
    return create_zero_dict(compute_dirns, acts1.shape[1])

  if compute_coefs:
    
    # also compute full coefficients over all neurons
    x_mask = jnp.dot(x_idxs.reshape((-1, 1)), x_idxs.reshape((1, -1)))
    y_mask = jnp.dot(y_idxs.reshape((-1, 1)), y_idxs.reshape((1, -1)))

    return_dict["coef_x"] = u.T
    return_dict["invsqrt_xx"] = invsqrt_xx
    return_dict["full_coef_x"] = np.zeros((numx, numx))
    np.place(return_dict["full_coef_x"], x_mask.to_py(),
             return_dict["coef_x"].to_py())
    return_dict["full_invsqrt_xx"] = np.zeros((numx, numx))
    np.place(return_dict["full_invsqrt_xx"], x_mask.to_py(),
             return_dict["invsqrt_xx"].to_py())
    
    return_dict["coef_y"] = v
    return_dict["invsqrt_yy"] = invsqrt_yy
    return_dict["full_coef_y"] = np.zeros((numy, numy))
    np.place(return_dict["full_coef_y"], y_mask.to_py(),
             return_dict["coef_y"].to_py())
    return_dict["full_invsqrt_yy"] = np.zeros((numy, numy))
    np.place(return_dict["full_invsqrt_yy"], y_mask.to_py(),
             return_dict["invsqrt_yy"].to_py())

    return_dict['full_coef_x'] = jnp.array(return_dict['full_coef_x'])
    return_dict['full_coef_y'] = jnp.array(return_dict['full_coef_y'])
    # compute means
    neuron_means1 = jnp.mean(acts1, axis=1, keepdims=True)
    neuron_means2 = jnp.mean(acts2, axis=1, keepdims=True)
    return_dict["neuron_means1"] = neuron_means1
    return_dict["neuron_means2"] = neuron_means2

  if compute_dirns:
    # orthonormal directions that are CCA directions
    cca_dirns1 = jnp.dot(jnp.dot(return_dict["full_coef_x"],
                               return_dict["full_invsqrt_xx"]),
                        (acts1 - neuron_means1)) + neuron_means1
    cca_dirns2 = jnp.dot(jnp.dot(return_dict["full_coef_y"],
  ...
  # summary statistics
  return_dict["mean"] = (jnp.mean(s[:idx1]), jnp.mean(s[:idx2]))
  return_dict["sum"] = (jnp.sum(s), jnp.sum(s))
...
def robust_cca_similarity(acts1, acts2, threshold=0.98, epsilon=1e-6,
  ...
  get_cca_similarity doesn't converge for some pair of ijnputs. This function
  ...
  for trial in range(num_cca_trials):
    ...
    except jnp.LinAlgError:
      acts1 = acts1*1e-1 + jnp.random.normal(size=acts1.shape)*epsilon
      acts2 = acts2*1e-1 + jnp.random.normal(size=acts1.shape)*epsilon
